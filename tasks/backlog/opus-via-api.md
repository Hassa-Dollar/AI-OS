# Backlog: move the Lead to the API — programmatic, budgeted, 24/7-capable

> Status: backlog (capture only) — AFTER SaaS 1 is proven. Raised by the operator. Will need an ADR.
> Do NOT implement before SaaS 1 ships and the ledger has real Lead-spend-per-task data.

## Idea
Today the Lead runs only as a human-invoked Cowork/Pro session — message-capped, and (by design) the
system cannot summon it. Move the Lead to its provider's API so the determinism layer can invoke the Lead
...
- **Precise spend control:** token-metered, per-call budgets, hard $ caps, context-size caps — fine-grained
  vs Pro's flat cap. Caveat (be honest): "more control" is NOT "cheaper by default" — API frontier-model pricing is costly per
...
- **A Lead budget governor** in the determinism layer: max $/day + max calls/window + context-size cap +
