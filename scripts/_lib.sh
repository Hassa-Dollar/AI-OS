...
# gen_inventory — print the AUTO-INVENTORY block (components → profile + built/stub, + available profiles),
...
gen_inventory() {
  local bt='`' d cn cprof src p pn cfound=0 pfound=0
  ...
  for d in components/*/; do
    ...
    if [[ -d "${d}src" ]]; then src="built"; else src="stub (no src/ yet)"; fi
    echo "- ${bt}${cn}${bt} → profile ${bt}${cprof:-?}${bt} · ${src}"; cfound=1
  done
