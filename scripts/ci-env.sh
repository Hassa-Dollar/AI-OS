...
# profile.sh apply copies the profile's ci-env.sh → scripts/ci-env.sh (ADR-0003); keep the two byte-identical.
...
export AUDIT_CMD="${AUDIT_CMD:-npm audit --omit=dev --audit-level=high || { sleep 30; npm audit --omit=dev --audit-level=high; } || { sleep 90; npm audit --omit=dev --audit-level=high; }}"   # AC8 + transient-registry retry (3 hits on 2026-07-15; same family as BUG-23)
