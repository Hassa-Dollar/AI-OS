...
<!-- AUTO-SHIPPED:BEGIN — generated by scripts/handoff.sh @ 2026-06-14T21:15:39Z; do not hand-edit -->
...
<!-- AUTO-STATE:BEGIN — generated by scripts/handoff.sh @ 2026-06-14T21:15:39Z; do not hand-edit -->
- **main:** `26f6a04 Merge pull request #10 from Hassa-Dollar/chore/gate-fixes`
- **checked-out branch:** `chore/restructure-v1` · worktree: DIRTY (expected right after handoff.sh ran — the refresh itself is the diff; it rides along with the next PR)
...
2026-06-11T17:13:56Z,dispatch,"002",main,hassa,"model=opencode-go/glm-5.1 verifier=opencode-go/kimi-k2.6 branch=task/002-method-not-allowed"
2026-06-11T17:27:12Z,qa,002,task/002-method-not-allowed,hassa,"verifier=opencode-go/kimi-k2.6 risk=low verdict=pass files=5 lines=219"
...
**In flight — the v1 restructure** (ADR-0002 components · ADR-0003 profiles + dynamic roles ·
ADR-0004 ergonomics; new contracts `os-component-boundary`, `profile.schema`). Goal: make the repo a
forkable scaffold whose product lives in an extractable `components/<name>/`, specialized by swappable
`profiles/<family>/<variant>/` that also carry the role→model bindings. Sequence: **ADRs/contracts (done)**
→ T-A handoff truthfulness · T-B operator-facing logs · T-C moderate cleanup → **T-D** move the service
into `components/<name>/` with isolation guardrails → **T-E** extract `profiles/web-app/ts-node-service`
and move pins into `profile.json`. T-D/T-E are Opus-gated. Tasks **000–002 shipped** (see §4).

---

## 6. Open follow-ups (Lead's to-do)

1. **Handoff currency** — §4's *facts* are now auto-generated (the `AUTO-SHIPPED` block, refreshed by
   `scripts/handoff.sh`); only the narrative in §4–§6 is hand-maintained. Refresh the narrative at session end.
2. ~~Verifier rotation for 002~~ — **done** (PR #9; Kimi K2.6 verified, rotated off DeepSeek).
3. **CI shellcheck** — `scripts/*.sh` are only `bash -n`-checked; consider a shellcheck step in `ci.yml`.
4. **v1 restructure (T-D / T-E)** — components move + profile/dynamic-role extraction; Opus-gated. See §5
   and the restructure plan from the session.

> Historical: the four follow-ups from the 001 runs landed in PR #7 — mandatory final-commit step
> (`prompts/task-execution.md` DO #5), read-only verifier with inline `TESTS_SUGGESTED`
> (`prompts/code-review.md`), dirty-worktree preflight in `gate.sh`, and completion-report + Working-Notes
> as implicit `files_allowed` allowances (AGENTS §3).
